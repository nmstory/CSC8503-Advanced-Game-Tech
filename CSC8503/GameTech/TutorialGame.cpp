...
void TutorialGame::UpdateKeys() {
	if (Window::GetKeyboard()->KeyPressed(KeyboardKeys::F1)) {
		InitWorldOriginal(); //We can reset the simulation at any time with F1
...
void TutorialGame::InitWorldOriginal() {
	world->ClearAndErase();
	physics->Clear();
	...
	AddCapsuleToWorld(Vector3(20.0f, 10.0f, 20.0f), 4, 2);
}

void TutorialGame::InitWorldSprings() {
	world->ClearAndErase();
	physics->Clear();
...
void TutorialGame::InitWorldConstraints() {
	world->ClearAndErase();
	physics->Clear();
...
void TutorialGame::InitWorldCollisions() {
	world->ClearAndErase();
	physics->Clear();
	...
	if (inSelectionMode) {
		...
		if (Window::GetMouse()->ButtonDown(NCL::MouseButtons::LEFT)) {
			...
			if (world->Raycast(ray, closestCollision, true)) {
				...
				// Fire forward collision (tutorial 1 further work Q.2)
